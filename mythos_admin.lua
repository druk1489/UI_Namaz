if _G.MYTHOS_LOADED and not _G.MYTHOS_DEBUG == true then
	-- error("Mythos is already running!",0)
	return
end

pcall(function() _G.MYTHOS_LOADED = true end)

COREGUI = game:GetService("CoreGui")
if not game:IsLoaded() then
	local notLoaded = Instance.new("Message")
	notLoaded.Parent = COREGUI
	notLoaded.Text = 'Mythos is waiting for the game to load'
	game.Loaded:Wait()
	notLoaded:Destroy()
end
...
PARENT = nil
if get_hidden_gui or game:GetService("CoreGui") then
	local hiddenUI = get_hidden_gui or game:GetService("CoreGui")
...
Title.TextSize = 18
Title.Text = "Mythos v" .. currentVersion
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- Mythos Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Invite to the Mythos support server.'}
...
CMDs[#CMDs + 1] = {NAME = 'reshade / shade / rtx [preset]', DESC = 'Reshade presets: off/low/soft/cinematic/ultra/retro/horror (CLIENT)'}
CMDs[#CMDs + 1] = {NAME = 'unreshade / unshade', DESC = 'Removes all Mythos shade effects + stops autodof'}
CMDs[#CMDs + 1] = {NAME = 'shadeinfo', DESC = 'Shows current Lighting + post-effect values'}
CMDs[#CMDs + 1] = {NAME = 'bloom [i] [size] [threshold] / unbloom', DESC = 'Fine BloomEffect tuning (CLIENT)'}
CMDs[#CMDs + 1] = {NAME = 'sunrays [i] [spread] / unsunrays', DESC = 'Fine SunRaysEffect tuning (CLIENT)'}
CMDs[#CMDs + 1] = {NAME = 'blur [size] / unblur', DESC = 'BlurEffect size 0-56 (CLIENT)'}
CMDs[#CMDs + 1] = {NAME = 'cc [bright] [contrast] [satur] [r g b] / uncc', DESC = 'ColorCorrection fine tuning (CLIENT)'}
CMDs[#CMDs + 1] = {NAME = 'dof [focus] [near] [far] [radius] / undof', DESC = 'DepthOfField manual focus (stops autodof)'}
CMDs[#CMDs + 1] = {NAME = 'autodof [on/off/smooth] / doffix', DESC = 'FIXED raycast autofocus (ignores character, no blur bug)'}
CMDs[#CMDs + 1] = {NAME = 'atmosphere [density] [haze] [glare] / unatmo', DESC = 'Atmosphere fine tuning (CLIENT)'}
CMDs[#CMDs + 1] = {NAME = 'exposure [num] / fogcolor [r g b] / fogstart [num]', DESC = 'ExposureCompensation + fog fine tuning'}
CMDs[#CMDs + 1] = {NAME = 'shadowsoft [0-1] / shadetech [future/shadowmap/voxel]', DESC = 'ShadowSoftness + Lighting.Technology'}
CMDs[#CMDs + 1] = {NAME = 'sky [stars] [sun] [moon] / inambient [r g b]', DESC = 'Sky + IndoorAmbient fine tuning'}
			...
			if cmd then
				...
				if infTimes then
					while lastBreakTime < cmdStartTime do
						...
						if not success and _G.MYTHOS_DEBUG then
    ...
    local function Load(Obj, Url)
        local function GiveOwnGlobals(Func, Script)
            ...
            FenvMt.__index = function(a,b)
                return RealFenv[b] == nil and _G[b] or RealFenv[b]
            end
...
-- ============================================================
-- MYTHOS SHADERS v2 : тонкие настройки + фикс AutoDOF raycast
-- Баг: старый raycast-DOF без фильтра бил в своего персонажа
-- (при ходьбе камера дёргается, луч цепляет голову/торс в 2-5
-- studs -> FocusDistance падал -> всё в блюре).
-- Фикс: Exclude-фильтр персонажа+камеры, старт луча +2 studs
-- вперёд, minDist-кламп, fallback на небо, lerp-сглаживание,
-- InFocusRadius >= 20 чтобы игрок оставался резким.
-- Весь модуль CLIENT (Lighting виден только тебе).
-- ============================================================
_G.MYTHOS_DOF = _G.MYTHOS_DOF or {conn = nil, on = false, minDist = 14, fallback = 850, maxDist = 2000, smooth = 0.18}
_G.MYTHOS_SHADE_BACKUP = _G.MYTHOS_SHADE_BACKUP or nil

local function mythosShadeBackup()
	if _G.MYTHOS_SHADE_BACKUP then return end
	_G.MYTHOS_SHADE_BACKUP = {}
	pcall(function()
		local L = Lighting
		_G.MYTHOS_SHADE_BACKUP.Ambient = L.Ambient
		_G.MYTHOS_SHADE_BACKUP.OutdoorAmbient = L.OutdoorAmbient
		_G.MYTHOS_SHADE_BACKUP.IndoorAmbient = L:FindFirstChild('IndoorAmbient') and L.IndoorAmbient or nil
		_G.MYTHOS_SHADE_BACKUP.Brightness = L.Brightness
		_G.MYTHOS_SHADE_BACKUP.ClockTime = L.ClockTime
		_G.MYTHOS_SHADE_BACKUP.FogColor = L.FogColor
		_G.MYTHOS_SHADE_BACKUP.FogEnd = L.FogEnd
		_G.MYTHOS_SHADE_BACKUP.FogStart = L.FogStart
		_G.MYTHOS_SHADE_BACKUP.GlobalShadows = L.GlobalShadows
		_G.MYTHOS_SHADE_BACKUP.ExposureCompensation = L.ExposureCompensation
		_G.MYTHOS_SHADE_BACKUP.ShadowSoftness = L.ShadowSoftness
		_G.MYTHOS_SHADE_BACKUP.GeographicLatitude = L.GeographicLatitude
		pcall(function() _G.MYTHOS_SHADE_BACKUP.EnvDiffuse = L.EnvironmentDiffuseScale end)
		pcall(function() _G.MYTHOS_SHADE_BACKUP.EnvSpec = L.EnvironmentSpecularScale end)
		pcall(function() _G.MYTHOS_SHADE_BACKUP.Tech = L.Technology end)
	end)
end

local function mythosShadeRestore()
	local b = _G.MYTHOS_SHADE_BACKUP
	...
	pcall(function()
		...
		if b.FogColor then Lighting.FogColor = b.FogColor end
		...
		if b.ExposureCompensation then Lighting.ExposureCompensation = b.ExposureCompensation end
		if b.ShadowSoftness then Lighting.ShadowSoftness = b.ShadowSoftness end
		if b.GeographicLatitude then Lighting.GeographicLatitude = b.GeographicLatitude end
		if b.EnvDiffuse then pcall(function() Lighting.EnvironmentDiffuseScale = b.EnvDiffuse end) end
		if b.EnvSpec then pcall(function() Lighting.EnvironmentSpecularScale = b.EnvSpec end) end
	end)
	_G.MYTHOS_SHADE_BACKUP = nil
end

local function mythosEnsure(className, tag)
	local got = Lighting:FindFirstChild(tag)
	if got and got.ClassName == className then return got end
	if got then pcall(function() got:Destroy() end) end
...
local function mythosDrop(tag)
	local e = Lighting:FindFirstChild(tag)
	if e then pcall(function() e:Destroy() end) end
end

local function mythosDropAllShades()
	for _, t in ipairs({'Mythos_Bloom','Mythos_Sun','Mythos_Blur','Mythos_CC','Mythos_DOF','Mythos_Atmo'}) do
		mythosDrop(t)
	end
end

local function mythosDofGet(create)
	local d = Lighting:FindFirstChild('Mythos_DOF')
	if d then return d end
	if create == false then return Lighting:FindFirstChildOfClass('DepthOfFieldEffect') end
	return mythosEnsure('DepthOfFieldEffect','Mythos_DOF')
end

local function mythosDofStop()
	_G.MYTHOS_DOF.on = false
	if _G.MYTHOS_DOF.conn then
		pcall(function() _G.MYTHOS_DOF.conn:Disconnect() end)
		_G.MYTHOS_DOF.conn = nil
	end
end

-- ФИКСИРОВАННЫЙ автофокус: игнорит персонажа, сглаживает, не блюрит всё при ходьбе
local function mythosDofStart(smooth)
	mythosDofStop()
	if smooth then _G.MYTHOS_DOF.smooth = math.clamp(tonumber(smooth) or 0.18, 0.02, 1) end
	local dof = mythosDofGet(true)
	pcall(function()
		dof.Enabled = true
		if (dof.InFocusRadius or 0) < 15 then dof.InFocusRadius = 25 end
		if (dof.NearIntensity or 0) == 0 then dof.NearIntensity = 0.25 end
		if (dof.FarIntensity or 0) == 0 then dof.FarIntensity = 0.55 end
		if (dof.FocusDistance or 0) < 1 then dof.FocusDistance = 120 end
	end)
	_G.MYTHOS_DOF.on = true
	_G.MYTHOS_DOF.conn = RunService.RenderStepped:Connect(function()
		pcall(function()
			local cam = workspace.CurrentCamera
			if not cam then return end
			...
			excl[#excl+1] = cam
			-- инструменты в руках тоже часть персонажа, но на всякий добавляем рюкзак-инструменты из воркспейса
			params.FilterDescendantsInstances = excl
			-- старт на 2 studs впереди: даже если фильтр проспал, не бьём в затылок в from-first-person
			local src = origin + (look * 2)
			local res = workspace:Raycast(src, look * 995, params)
			local target
			if res and res.Position then
				target = (origin - res.Position).Magnitude
				-- ГЛАВНЫЙ ФИКС: дистанция меньше minDist = луч всё равно задел своё (голова/торс/аксессуар) -> считаем как небо
				if target < _G.MYTHOS_DOF.minDist then
					target = _G.MYTHOS_DOF.fallback
				end
			else
				target = _G.MYTHOS_DOF.fallback -- небо / нет попадания = бесконечность
			end
			target = math.clamp(target, _G.MYTHOS_DOF.minDist, _G.MYTHOS_DOF.maxDist)
			local cur = dof.FocusDistance
			if typeof(cur) ~= 'number' then cur = target end
			-- lerp чтобы при ходьбе фокус не прыгал и не было резкого блюра
			dof.FocusDistance = cur + (target - cur) * _G.MYTHOS_DOF.smooth
		end)
	end)
end

addcmd('reshade',{'shade','rtx','graphics'},function(args, speaker)
	local p = (args[1] or 'cinematic'):lower()
	if p == 'off' or p == 'none' or p == '0' then execCmd('unreshade'); return end
	mythosShadeBackup()
	mythosDofStop()
	pcall(function()
		local L = Lighting
		if p == 'low' or p == 'potato' or p == 'fps' then
			mythosDropAllShades()
			...
			notify('Reshade','potato/low: всё выкл для FPS')
		elseif p == 'soft' then
			L.GlobalShadows = true
			L.Brightness = 2
			L.ExposureCompensation = 0.2
			L.ShadowSoftness = 0.5
			local b = mythosEnsure('BloomEffect','Mythos_Bloom'); b.Enabled = true; b.Intensity = 0.4; b.Size = 25; b.Threshold = 1.2
			local s = mythosEnsure('SunRaysEffect','Mythos_Sun'); s.Enabled = true; s.Intensity = 0.08; s.Spread = 0.8
			local c = mythosEnsure('ColorCorrectionEffect','Mythos_CC'); c.Enabled = true; c.Brightness = 0.03; c.Contrast = 0.08; c.Saturation = 0.15; c.TintColor = Color3.fromRGB(255,255,255)
			mythosDrop('Mythos_Blur'); mythosDrop('Mythos_DOF')
			notify('Reshade','soft применён')
		elseif p == 'retro' or p == 'vhs' then
			L.GlobalShadows = true
			L.ExposureCompensation = -0.1
			local b = mythosEnsure('BloomEffect','Mythos_Bloom'); b.Enabled = true; b.Intensity = 0.25; b.Size = 18; b.Threshold = 1.4
			mythosDrop('Mythos_Sun'); mythosDrop('Mythos_Blur')
			local c = mythosEnsure('ColorCorrectionEffect','Mythos_CC'); c.Enabled = true; c.Brightness = -0.02; c.Contrast = 0.3; c.Saturation = -0.3; c.TintColor = Color3.fromRGB(255,230,200)
			mythosDrop('Mythos_DOF')
			notify('Reshade','retro/vhs применён')
		elseif p == 'horror' or p == 'dark' then
			L.GlobalShadows = true
			L.Ambient = Color3.fromRGB(40,40,50)
			L.OutdoorAmbient = Color3.fromRGB(50,50,60)
			L.Brightness = 1
			L.ExposureCompensation = -0.5
			L.FogColor = Color3.fromRGB(10,10,12)
			L.FogEnd = 250; L.FogStart = 20
			mythosDrop('Mythos_Bloom'); mythosDrop('Mythos_Sun'); mythosDrop('Mythos_Blur')
			local c = mythosEnsure('ColorCorrectionEffect','Mythos_CC'); c.Enabled = true; c.Brightness = -0.08; c.Contrast = 0.3; c.Saturation = -0.6; c.TintColor = Color3.fromRGB(200,200,220)
			mythosDrop('Mythos_DOF')
			notify('Reshade','horror применён')
		elseif p == 'ultra' or p == 'rtx' or p == 'max' then
			L.GlobalShadows = true
			L.Brightness = 2
			...
			local b = mythosEnsure('BloomEffect','Mythos_Bloom'); b.Enabled = true; b.Intensity = 1.1; b.Size = 45; b.Threshold = 0.9
			local s = mythosEnsure('SunRaysEffect','Mythos_Sun'); s.Enabled = true; s.Intensity = 0.35; s.Spread = 0.6
			local c = mythosEnsure('ColorCorrectionEffect','Mythos_CC'); c.Enabled = true; c.Brightness = 0.02; c.Contrast = 0.2; c.Saturation = 0.25; c.TintColor = Color3.fromRGB(255,255,255)
			local a = mythosEnsure('Atmosphere','Mythos_Atmo'); a.Density = 0.4; a.Haze = 3; a.Glare = 0; a.Decay = Color3.fromRGB(120,140,180)
			local d = mythosDofGet(true); d.Enabled = true; d.FocusDistance = 120; d.NearIntensity = 0.25; d.FarIntensity = 0.6; d.InFocusRadius = 30
			mythosDofStart(0.15)
			notify('Reshade','ultra/rtx + FIXED autodof вкл')
		else -- cinematic default
			L.GlobalShadows = true
			L.Brightness = 2
			L.ExposureCompensation = 0.3
			L.ShadowSoftness = 0.3
			local b = mythosEnsure('BloomEffect','Mythos_Bloom'); b.Enabled = true; b.Intensity = 0.8; b.Size = 30; b.Threshold = 1.0
			local s = mythosEnsure('SunRaysEffect','Mythos_Sun'); s.Enabled = true; s.Intensity = 0.22; s.Spread = 0.7
			local c = mythosEnsure('ColorCorrectionEffect','Mythos_CC'); c.Enabled = true; c.Brightness = 0.02; c.Contrast = 0.15; c.Saturation = 0.2; c.TintColor = Color3.fromRGB(255,252,245)
			local d = mythosDofGet(true); d.Enabled = true; d.FocusDistance = 120; d.NearIntensity = 0.2; d.FarIntensity = 0.5; d.InFocusRadius = 30
			mythosDofStart(0.18)
			notify('Reshade','cinematic + FIXED autodof вкл (dof off если не нужен)')
		end
	end)
end)

addcmd('unreshade',{'unshade','nortx','reshadeoff','shadeoff'},function(args, speaker)
	mythosDofStop()
	mythosDropAllShades()
	mythosShadeRestore()
	notify('Reshade','Выкл: эффекты удалены, Lighting откачен')
end)

addcmd('shadeinfo',{'shadehelp','reshadeinfo','gfxinfo'},function(args, speaker)
	local t = {}
	pcall(function()
		t[#t+1] = 'Clock='..tostring(Lighting.ClockTime)
		t[#t+1] = 'Bright='..tostring(Lighting.Brightness)
		t[#t+1] = 'Exp='..tostring(Lighting.ExposureCompensation)
		t[#t+1] = 'Fog='..tostring(Lighting.FogStart)..'/'..tostring(Lighting.FogEnd)
		t[#t+1] = 'ShadowSoft='..tostring(Lighting.ShadowSoftness)
		for _, e in ipairs(Lighting:GetChildren()) do
			if e:IsA('BloomEffect') then t[#t+1] = 'Bloom('..e.Name..') i='..tostring(e.Intensity)..' s='..tostring(e.Size)..' t='..tostring(e.Threshold) end
			if e:IsA('DepthOfFieldEffect') then t[#t+1] = 'DOF('..e.Name..') f='..string.format('%.1f',e.FocusDistance)..' n='..tostring(e.NearIntensity)..' far='..tostring(e.FarIntensity)..' r='..tostring(e.InFocusRadius)..( _G.MYTHOS_DOF.on and ' AUTO' or '') end
			if e:IsA('ColorCorrectionEffect') then t[#t+1] = 'CC('..e.Name..') b='..tostring(e.Brightness)..' c='..tostring(e.Contrast)..' s='..tostring(e.Saturation) end
			if e:IsA('SunRaysEffect') then t[#t+1] = 'Sun('..e.Name..') i='..tostring(e.Intensity)..' spread='..tostring(e.Spread) end
			if e:IsA('BlurEffect') then t[#t+1] = 'Blur('..e.Name..') size='..tostring(e.Size) end
			if e:IsA('Atmosphere') then t[#t+1] = 'Atmo('..e.Name..') d='..tostring(e.Density)..' haze='..tostring(e.Haze)..' glare='..tostring(e.Glare) end
		end
	end)
	local msg = table.concat(t, ' | ')
	print('[Mythos ShadeInfo] '..msg)
	notify('ShadeInfo', msg:sub(1, 220))
end)

addcmd('bloom',{'shadebloom'},function(args, speaker)
	local i = tonumber(args[1]) or 0.8
	local s = tonumber(args[2]) or 30
	local th = tonumber(args[3]) or 1.0
	local b = mythosEnsure('BloomEffect','Mythos_Bloom')
	b.Enabled = true
	b.Intensity = math.clamp(i, 0, 10)
	b.Size = math.clamp(s, 1, 56)
	b.Threshold = math.clamp(th, 0, 2)
	notify('Bloom','i='..b.Intensity..' size='..b.Size..' thr='..b.Threshold)
end)

addcmd('unbloom',{'nobloom'},function(args, speaker)
	mythosDrop('Mythos_Bloom')
	notify('Bloom','Выкл (Mythos_Bloom удалён)')
end)

addcmd('sunrays',{'sun','shadesun'},function(args, speaker)
	local i = tonumber(args[1])
	if i == nil then i = 0.2 end
	local sp = tonumber(args[2])
	if sp == nil then sp = 0.7 end
	local e = mythosEnsure('SunRaysEffect','Mythos_Sun')
	e.Enabled = true
	e.Intensity = math.clamp(i, 0, 2)
	e.Spread = math.clamp(sp, 0, 1)
	notify('SunRays','i='..e.Intensity..' spread='..e.Spread)
end)

addcmd('unsunrays',{'nosunrays','nosun'},function(args, speaker)
	mythosDrop('Mythos_Sun')
	notify('SunRays','Выкл')
end)

addcmd('blur',{'shadeblur'},function(args, speaker)
	local s = tonumber(args[1]) or 12
	local e = mythosEnsure('BlurEffect','Mythos_Blur')
	e.Enabled = true
	e.Size = math.clamp(s, 0, 56)
	notify('Blur','size='..e.Size)
end)

addcmd('unblur',{'noblur'},function(args, speaker)
	mythosDrop('Mythos_Blur')
	for _, v in ipairs(Lighting:GetChildren()) do
		if v:IsA('BlurEffect') and v.Name ~= 'Mythos_Blur' then pcall(function() v.Enabled = false end) end
	end
	notify('Blur','Выкл')
end)

addcmd('cc',{'colorcorrection','colorgrade','shadecc'},function(args, speaker)
	local br = tonumber(args[1]) or 0.02
	local ct = tonumber(args[2]) or 0.15
	local st = tonumber(args[3]) or 0.2
	local e = mythosEnsure('ColorCorrectionEffect','Mythos_CC')
	e.Enabled = true
	e.Brightness = math.clamp(br, -1, 1)
	e.Contrast = math.clamp(ct, -1, 2)
	e.Saturation = math.clamp(st, -1, 3)
	if args[4] and args[5] and args[6] then
		local r = math.clamp(tonumber(args[4]) or 255, 0, 255)
		local g = math.clamp(tonumber(args[5]) or 255, 0, 255)
		local bb = math.clamp(tonumber(args[6]) or 255, 0, 255)
		e.TintColor = Color3.fromRGB(r, g, bb)
	end
	notify('ColorCorrection','b='..e.Brightness..' c='..e.Contrast..' s='..e.Saturation)
end)

addcmd('uncc',{'nocc','nocolor'},function(args, speaker)
	mythosDrop('Mythos_CC')
	notify('ColorCorrection','Выкл')
end)

addcmd('dof',{'depthoffield','depth','focus'},function(args, speaker)
	mythosDofStop() -- ручной фокус всегда гасит авто, чтобы не было борьбы
	local f = tonumber(args[1]) or 120
	local n = tonumber(args[2]) or 0.25
	local fr = tonumber(args[3]) or 0.55
	local r = tonumber(args[4]) or 30
	local d = mythosDofGet(true)
	d.Enabled = true
	d.FocusDistance = math.clamp(f, 0.1, 5000)
	d.NearIntensity = math.clamp(n, 0, 1)
	d.FarIntensity = math.clamp(fr, 0, 1)
	d.InFocusRadius = math.clamp(r, 1, 200)
	notify('DOF','ручной: focus='..d.FocusDistance..' (autodof выкл)')
end)

addcmd('undof',{'nodof','undoff','nodoff'},function(args, speaker)
	mythosDofStop()
	mythosDrop('Mythos_DOF')
	notify('DOF','Выкл')
end)

addcmd('autodof',{'dofauto','autofocus'},function(args, speaker)
	local m = (args[1] or 'on'):lower()
	if m == 'off' or m == 'stop' or m == '0' then
		mythosDofStop()
		notify('AutoDOF','Выкл (фокус заморожен)')
		return
	end
	if m == 'min' and args[2] then
		_G.MYTHOS_DOF.minDist = math.clamp(tonumber(args[2]) or 14, 2, 100)
		notify('AutoDOF','minDist='.._G.MYTHOS_DOF.minDist)
		return
	end
	local sm = tonumber(args[1]) or tonumber(args[2]) or 0.18
	mythosDofStart(sm)
	notify('AutoDOF','ВКЛ (FIXED): игнор персонажа, min='.._G.MYTHOS_DOF.minDist..' smooth='.._G.MYTHOS_DOF.smooth)
end)

addcmd('doffix',{'fixdof','undofblur','noblurbug'},function(args, speaker)
	-- экстренный фикс "всё в блюре когда иду": сброс + перезапуск правильного автофокуса
	mythosDofStop()
	local d = mythosDofGet(true)
	pcall(function()
		d.Enabled = true
	...
	_G.MYTHOS_DOF.minDist = 14
	_G.MYTHOS_DOF.fallback = 850
	_G.MYTHOS_DOF.smooth = 0.18
	mythosDofStart(0.18)
	notify('DOFFIX','Починено: фокус 150, радиус 30, автофокус перезапущен БЕЗ задевания персонажа')
end)

addcmd('atmosphere',{'atmo','shadeatmo','haze'},function(args, speaker)
	local d = tonumber(args[1]) or 0.35
	local h = tonumber(args[2]) or 2
	local g = tonumber(args[3]) or 0
	local e = mythosEnsure('Atmosphere','Mythos_Atmo')
	e.Density = math.clamp(d, 0, 1)
	e.Haze = math.clamp(h, 0, 10)
	e.Glare = math.clamp(g, 0, 10)
	if args[4] then e.Offset = math.clamp(tonumber(args[4]) or 0.2, 0, 1) end
	if args[5] and args[6] and args[7] then
		e.Color = Color3.fromRGB(math.clamp(tonumber(args[5]) or 200,0,255), math.clamp(tonumber(args[6]) or 200,0,255), math.clamp(tonumber(args[7]) or 200,0,255))
	end
	notify('Atmosphere','d='..e.Density..' haze='..e.Haze..' glare='..e.Glare)
end)

addcmd('unatmo',{'noatmo','noatmosphere'},function(args, speaker)
	mythosDrop('Mythos_Atmo')
	notify('Atmosphere','Выкл')
end)

addcmd('exposure',{'exp','shadeexp'},function(args, speaker)
	local v = tonumber(args[1])
	if not v then return notify('Exposure','юзай: exposure [-3..3]') end
	Lighting.ExposureCompensation = math.clamp(v, -3, 3)
	notify('Exposure', tostring(Lighting.ExposureCompensation))
end)

addcmd('fogcolor',{'fogcolour','shadefogcolor'},function(args, speaker)
	local r = math.clamp(tonumber(args[1]) or 128, 0, 255)
	local g = math.clamp(tonumber(args[2]) or 128, 0, 255)
	local b = math.clamp(tonumber(args[3]) or 128, 0, 255)
	Lighting.FogColor = Color3.fromRGB(r, g, b)
	notify('FogColor', r..','..g..','..b)
end)

addcmd('fogstart',{'fogst'},function(args, speaker)
	local v = tonumber(args[1])
	if not v then return notify('FogStart','юзай: fogstart [num]') end
	Lighting.FogStart = v
	notify('FogStart', tostring(v))
end)

addcmd('shadowsoft',{'softshadow','shadowsoftness'},function(args, speaker)
	local v = tonumber(args[1]) or 0.3
	Lighting.ShadowSoftness = math.clamp(v, 0, 1)
	notify('ShadowSoftness', tostring(Lighting.ShadowSoftness))
end)

addcmd('shadetech',{'tech','shadelighttech'},function(args, speaker)
	local m = (args[1] or ''):lower()
	local ok = pcall(function()
		if m == 'future' then Lighting.Technology = Enum.Technology.Future
		elseif m == 'shadowmap' or m == 'shadow' then Lighting.Technology = Enum.Technology.ShadowMap
		elseif m == 'voxel' then Lighting.Technology = Enum.Technology.Voxel
		elseif m == 'legacy' then Lighting.Technology = Enum.Technology.Legacy
		else return false end
		return true
	end)
	if ok then notify('Tech','Technology='..tostring(Lighting.Technology)) else notify('Tech','юзай: shadetech [future/shadowmap/voxel/legacy]') end
end)

addcmd('sky',{'shadesky','stars'},function(args, speaker)
	local sky = Lighting:FindFirstChildOfClass('Sky')
	if not sky then return notify('Sky','на карте нет Sky') end
	if args[1] then pcall(function() sky.StarCount = math.clamp(tonumber(args[1]) or 3000, 0, 5000) end) end
	if args[2] then pcall(function() sky.SunAngularSize = math.clamp(tonumber(args[2]) or 11, 0, 60) end) end
	if args[3] then pcall(function() sky.MoonAngularSize = math.clamp(tonumber(args[3]) or 11, 0, 60) end) end
	notify('Sky','stars='..tostring(sky.StarCount)..' sun='..tostring(sky.SunAngularSize)..' moon='..tostring(sky.MoonAngularSize))
end)

addcmd('inambient',{'indoorambient','inamb'},function(args, speaker)
	local r = math.clamp(tonumber(args[1]) or 128, 0, 255)
	local g = math.clamp(tonumber(args[2]) or 128, 0, 255)
	local b = math.clamp(tonumber(args[3]) or 128, 0, 255)
	pcall(function() Lighting.IndoorAmbient = Color3.fromRGB(r, g, b) end)
	notify('IndoorAmbient', r..','..g..','..b)
end)
...
task.spawn(function()
	...
	minimizeHolder()
	if table.find({Enum.Platform.IOS, Enum.Platform.Android}, UserInputService:GetPlatform()) then notify("Unstable Device", "On mobile, Mythos may have issues or features that are not functioning correctly.") end
end)
-- ================================================================
-- MYTHOS ANTICHEAT v2 — полный модуль
-- fling-торс, дождь из парт, partcontrol, детект аимбот/флай/спид
-- automarkcheat, unmarkall, защита от сидячих/транспорт/АФК
-- ================================================================
...
local function getUnanchoredWorldParts()
    local parts = {}
    for _, v in ipairs(workspace:GetDescendants()) do
        if v:IsA("BasePart") and not v.Anchored then
            local isChar, _ = isCharPart(v)
            if not isChar then
...
-- Лог
-- ═══════════════════════════════════════════════════════════════
local LOG_FOLDER = "Mythos"
local LOG_FILE   = LOG_FOLDER .. "/cheaters.json"
local function ensureFolder()
    if isfolder and not isfolder(LOG_FOLDER) then makefolder(LOG_FOLDER) end
end
local function loadLog()
    ensureFolder()
    if readfile and isfile and isfile(LOG_FILE) then
        local ok, d = pcall(function() return HttpService:JSONDecode(readfile(LOG_FILE)) end)
        if ok and type(d) == "table" then return d end
    end
    return {}
end
local function saveLog(t)
    ensureFolder()
    if writefile then
        local ok, enc = pcall(function() return HttpService:JSONEncode(t) end)
        if ok then writefile(LOG_FILE, enc) end
    end
end
local function addToLog(player, reason)
    local log = loadLog()
...
-- Таблица помеченных
-- ═══════════════════════════════════════════════════════════════
local markedCheaters = {}  -- [userId] = { player, conns={}, reason }
local function isMarked(p) return markedCheaters[p.UserId] ~= nil end

-- ═══════════════════════════════════════════════════════════════
-- МЕТОДЫ НАКАЗАНИЯ
-- ═══════════════════════════════════════════════════════════════

-- 1. FLING — BodyAngularVelocity на HRP цели (логика из /fling)
local function punishFling(target)
    local char = target.Character; if not char then return end
    local root = getRoot(char); if not root then return end
    -- установить физику как в fling
    ...
    for _, c in ipairs(root:GetChildren()) do
        if c:IsA("BodyAngularVelocity") and c.Name == "MYTHOS_FLING" then c:Destroy() end
    end
    local bav = Instance.new("BodyAngularVelocity")
    bav.Name            = "MYTHOS_FLING"
    bav.AngularVelocity = Vector3.new(0, 99999, 0)
    bav.MaxTorque       = Vector3.new(0, math.huge, 0)
    bav.P               = math.huge
    bav.Parent          = root
    -- пульс: чередуем направление для максимального хаоса
    task.spawn(function()
        local tick = 0
        while bav and bav.Parent and isMarked(target) do
            tick = tick + 1
            bav.AngularVelocity = Vector3.new(
                (tick % 3 == 0) and 50000 or 0,
                tick % 2 == 0 and 99999 or -99999,
                (tick % 4 == 0) and 30000 or 0
            )
            task.wait(0.15)
        end
...
-- Убрать флинг (восстановить физику)
local function removeFling(target)
    local char = target.Character; if not char then return end
...
-- 2. PART RAIN — собираем все unanchored парты и кидаем на цель
local function punishPartRain(target)
    local char = target.Character; if not char then return end
    local root = getRoot(char); if not root then return end
    local parts = getUnanchoredWorldParts()
    ...
    for _, p in ipairs(parts) do
        pcall(function()
            ...
            p.CFrame   = CFrame.new(center.Position + offset)
...
-- 3. Убить (здоровье 0)
local function punishKill(target)
    local char = target.Character; if not char then return end
    local hum = getHuman(char)
    if hum then hum.Health = 0 end
end

-- 4. Заморозить скорость
local function punishFreeze(target)
    local char = target.Character; if not char then return end
    local hum = getHuman(char)
...
-- 5. BodyForce взрыв
local function punishBlast(target)
    local char = target.Character; if not char then return end
    for _, p in ipairs(char:GetChildren()) do
        if p:IsA("BasePart") then
            local bf = Instance.new("BodyForce")
            bf.Force  = Vector3.new(math.random(-60000,60000), math.random(80000,160000), math.random(-60000,60000))
            bf.Parent = p
...
-- 6. Телепорт в войд
local function punishVoid(target)
    local char = target.Character; if not char then return end
    local root = getRoot(char)
    if root then root.CFrame = CFrame.new(0, -5000, 0) end
end

-- 7. Трип
local function punishTrip(target)
    local char = target.Character; if not char then return end
    local root = getRoot(char); local hum = getHuman(char)
    if root and hum then
        hum:ChangeState(0)
        root.Velocity = Vector3.new(math.random(-1,1)*120, math.random(60,200), math.random(-1,1)*120)
    end
end

local PUNISH_SEQ = { punishFling, punishPartRain, punishKill, punishBlast, punishTrip, punishFreeze, punishVoid }

local function startPunishLoop(player)
    local entry = markedCheaters[player.UserId]; if not entry then return end
    local t = task.spawn(function()
        local i = 1
        while markedCheaters[player.UserId] do
            pcall(PUNISH_SEQ[i], player)
            i = (i % #PUNISH_SEQ) + 1
            task.wait(0.3)
        end
    end)
    local cc = player.CharacterAdded:Connect(function()
        if isMarked(player) then
            task.wait(0.05)
            pcall(punishFling,   player)
            pcall(punishFreeze,  player)
        end
    end)
    table.insert(entry.conns, cc)
end

local function stopPunish(player)
    local entry = markedCheaters[player.UserId]; if not entry then return end
    pcall(removeFling, player)
    for _, c in ipairs(entry.conns or {}) do pcall(function() c:Disconnect() end) end
    markedCheaters[player.UserId] = nil
end

-- ═══════════════════════════════════════════════════════════════
-- КОМАНДЫ: markcheat / unmarkcheat / listcheats / unmarkall
-- ═══════════════════════════════════════════════════════════════
CMDs[#CMDs+1] = {NAME='markcheat [player]',   DESC='[AC] Пометить читера — fling + rain + kill loop'}
CMDs[#CMDs+1] = {NAME='unmarkcheat [player]', DESC='[AC] Снять метку с игрока'}
CMDs[#CMDs+1] = {NAME='unmarkall',            DESC='[AC] Снять ВСЕ метки и остановить систему'}
...
CMDs[#CMDs+1] = {NAME='partrain [player]',    DESC='[AC] Кинуть все unanchored парты в игрока'}
CMDs[#CMDs+1] = {NAME='partcontrol',          DESC='[AC] Управление всеми unanchored партами (курсор/формы)'}

addcmd('markcheat',{'mc','cheat','markc'},function(args,speaker)
    local players = getPlayer(args[1],speaker)
    for _,name in ipairs(players) do
        local t = Players[name]
        if not t then notify('Mythos AC','Не найден: '..tostring(name))
        elseif t==lp then notify('Mythos AC','Нельзя себя')
        elseif isMarked(t) then notify('Mythos AC',t.Name..' уже помечен')
        else
            markedCheaters[t.UserId]={player=t,conns={},reason='manual'}
            addToLog(t,'manual')
            startPunishLoop(t)
...
addcmd('unmarkcheat',{'umc','uncheat'},function(args,speaker)
    ...
    for _,name in ipairs(players) do
        local t = Players[name]
        if t and isMarked(t) then stopPunish(t); notify('Mythos AC','✅ '..t.Name..' снят') end
    end
end)

addcmd('unmarkall',{'clearmarks','stopac'},function(args,speaker)
    local n = 0
    for uid, entry in pairs(markedCheaters) do
        pcall(stopPunish, entry.player); n=n+1
    end
    markedCheaters = {}
    autoDetectEnabled = false
    notify('Mythos AC','Снято меток: '..n..'. Авто-детект выкл.')
end)

addcmd('listcheats',{'lc','cheats'},function(args,speaker)
    local list={}
    for _,e in pairs(markedCheaters) do
...
addcmd('loadcheats',{'cheatslog'},function(args,speaker)
    ...
    notify('Mythos AC — Лог ('..#log..')',table.concat(lines,'\n'))
end)

-- ═══════════════════════════════════════════════════════════════
-- PARTRAIN — ручная команда
-- ═══════════════════════════════════════════════════════════════
addcmd('partrain',{'prain'},function(args,speaker)
    ...
    for _,name in ipairs(players) do
        ...
        if t then
            local count=0
            pcall(function() punishPartRain(t); count=count+1 end)
            notify('Mythos','Part rain → '..t.Name)
        end
    end
end)

-- ═══════════════════════════════════════════════════════════════
-- PARTCONTROL — управление unanchored партами курсором / формами
-- ═══════════════════════════════════════════════════════════════
...
local PC_FORMS = {
    ...
    -- cube grid
    function(parts, origin)
        local side = math.ceil(#parts^(1/3))
        local i    = _G._PC_IDX or 0
        _G._PC_IDX = i + 1
        local x = i % side
        local y = math.floor(i/side) % side
        local z = math.floor(i/(side*side)) % side
        local s = 4
        return Vector3.new(origin.X + x*s - side*s/2,
                           origin.Y + y*s,
                           origin.Z + z*s - side*s/2)
    end,
    -- tornado spiral
    function(parts, origin)
        local i    = _G._PC_IDX or 0
        _G._PC_IDX = i + 1
        local angle = i * 0.4 + os.clock() * 2
        local height = i * 0.3
        local r = 6
        return Vector3.new(
    ...
    -- follow cursor (mouse hit position)
    function(parts, origin)
        local i    = _G._PC_IDX or 0
        _G._PC_IDX = i + 1
        local spread = 2
...
local FORM_NAMES = {"Сфера","Куб","Торнадо","Курсор (кластер)"}

local function pcCleanup()
    for part, bodies in pairs(pcBodies) do
        pcall(function() bodies.bp:Destroy() end)
        pcall(function() bodies.bg:Destroy() end)
    end
    pcBodies = {}; pcParts = {}
end

local function pcStartControl()
    pcCleanup()
    pcParts = getUnanchoredWorldParts()
    if #pcParts == 0 then
        notify('Mythos PartControl','Нет unanchored парт в мире'); return
    end
    for _, p in ipairs(pcParts) do
        local bp = Instance.new("BodyPosition")
        bp.MaxForce = Vector3.new(1e6,1e6,1e6); bp.P=8000; bp.D=500
        bp.Position = p.Position
        bp.Parent   = p
        local bg = Instance.new("BodyGyro")
        bg.MaxTorque = Vector3.new(1e5,1e5,1e5); bg.P=5000; bg.D=200
        bg.CFrame    = p.CFrame
        bg.Parent    = p
        pcBodies[p] = {bp=bp, bg=bg}
    end
    pcActive = true
    notify('Mythos PartControl','▶ '..#pcParts..' парт захвачено. Форма: '..FORM_NAMES[pcFormIndex+1 > #FORM_NAMES and 1 or pcFormIndex+1])
    task.spawn(function()
        while pcActive do
            ...
            local formFn = PC_FORMS[(pcFormIndex % #PC_FORMS) + 1]
            _G._PC_IDX = 0
            for i, p in ipairs(pcParts) do
                if pcBodies[p] and pcBodies[p].bp and pcBodies[p].bp.Parent then
                    _G._PC_IDX = i - 1
                    local target = pcall and (function()
                        local ok,v = pcall(formFn, pcParts, origin)
                        return ok and v or nil
                    end)() or nil
                    if target then
                        pcBodies[p].bp.Position = target
                        pcBodies[p].bg.CFrame   = CFrame.new(target)
                    end
                end
            end
            task.wait(0.04)
...
addcmd('partcontrol',{'pc','partctrl'},function(args,speaker)
    if pcActive then
        -- следующая форма или выключить
        if args[1] == "stop" or args[1] == "off" then
            pcActive = false; pcCleanup()
            notify('Mythos PartControl','■ Остановлено')
        elseif args[1] == "next" or args[1] == "form" then
            pcFormIndex = (pcFormIndex + 1) % #PC_FORMS
            notify('Mythos PartControl','Форма: '..FORM_NAMES[pcFormIndex+1])
        else
            pcActive = false; pcCleanup()
            task.wait(0.1)
            pcStartControl()
...
addcmd('stoppartcontrol',{'stoppc','nopc'},function(args,speaker)
    pcActive=false; pcCleanup()
    notify('Mythos PartControl','■ Остановлено')
end)

-- ═══════════════════════════════════════════════════════════════
-- АВТО-ДЕТЕКТ  (automarkcheat)
-- ═══════════════════════════════════════════════════════════════
local autoDetectEnabled = false

-- Пороги
local SPEED_THRESH   = 85    -- studs/sec горизонт. (WalkSpeed обычно ≤32)
local FLY_AIR_TIME   = 4.0   -- секунд в воздухе + горизонт движение = fly
local FLY_HORZ_MIN   = 18    -- горизонт. скорость при флае
local TELE_DIST      = 190   -- studs прыжок за тик = телепорт
local WS_HARD        = 110   -- WalkSpeed напрямую
...
local function checkAimbot(target)
    ...
    local dir    = head.CFrame.LookVector * AIMBOT_DIST
    ...
    local paramsChar = RaycastParams.new()
    paramsChar.FilterType             = Enum.RaycastFilterType.Include
    paramsChar.FilterDescendantsInstances = charParts

    -- Луч 1: в ЧТО попадаем первым (всё)
    local hitAll  = workspace:Raycast(origin, dir, paramsAll)
    -- Луч 2: в какого игрока попадаем (если вообще)
    local hitChar = workspace:Raycast(origin, dir, paramsChar)
...
-- Основная функция детекта для одного игрока
-- Возвращает строку причины или nil
local function detectPlayer(target)
    if target == lp then return nil end
    if isMarked(target) then return nil end
    ...
    if not s.isAfk then
        ...
        if throughWall then
            ...
            -- постепенно снижаем streak (не сразу, чтобы не было сброса от одного кадра)
            if s.aimbotStreak > 0 then s.aimbotStreak = s.aimbotStreak - 1 end
        end
    else
        -- АФК — сбрасываем streak (луч «гаснет»)
        s.aimbotStreak = 0
    end
...
local function runAutoDetect()
    while autoDetectEnabled do
        for _, target in ipairs(Players:GetPlayers()) do
            if target ~= lp and not isMarked(target) then
                local ok, reason = pcall(detectPlayer, target)
                local r = ok and reason or nil
                if r then
                    markedCheaters[target.UserId] = {
                        player=target, conns={}, reason=r
                    }
                    addToLog(target, r)
                    startPunishLoop(target)
                    notify('⚠ Mythos — ЧИТЕР!', target.Name .. '\n' .. r)
                end
            end
        end
        task.wait(CHECK_DT)
...
addcmd('automarkcheat',{'autoac','autocheats','anticheat','ac'},function(args,speaker)
    autoDetectEnabled = not autoDetectEnabled
    if autoDetectEnabled then
        task.spawn(runAutoDetect)
...
-- Очищаем стейт когда игрок выходит
Players.PlayerRemoving:Connect(function(p)
    playerState[p.UserId] = nil
    if isMarked(p) then stopPunish(p) end
end)
